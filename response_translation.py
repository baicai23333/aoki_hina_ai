...
FIXED_IDENTITY_RESPONSE = IDENTITY_RESPONSE.japanese
FIXED_PRIVATE_RESPONSE = PRIVATE_RESPONSE.japanese
FIXED_INSUFFICIENT_EVIDENCE_RESPONSE = INSUFFICIENT_EVIDENCE_RESPONSE.japanese
...
class ResponseTranslationService:
    ...
    def _fixed_response(
        ...
        return fixed_safety_response(intent, boundary_action)
    ...
    def _deterministic_issues(source_text: str, candidate: str) -> tuple[str, ...]:
        ...
        if not candidate.strip():
            ...
            return tuple(issues)
        if has_hidden_or_redacted_content(candidate):
            issues.append("translation_hidden_or_redacted")
...
source_text 是最终实际展示的唯一正文；不得猜测、补全或恢复其中没有出现或已经删除、隐藏、屏蔽的内容。
只输出自然的日语译文。"""
        if repair_issues:
            repair_lines = [
...
source_text 是最终实际展示的唯一正文；如果 candidate_translation 猜测、补全或恢复了 source_text 中没有出现的内容，必须拒绝。
